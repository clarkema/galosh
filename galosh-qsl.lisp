...
(defun print-qso-details (q qrz-details)
  (labels ((qrz (field)
    ...
    (mvprintw 10 1 (format nil "             ~10A ~10A ~10A~%"   "Mine" "His"  "QRZ"))
    (mvprintw 11 1 (format nil "CQ Zone:     ~10A ~10A ~10A~%"   (n->es (q-my-cq-zone q))  (n->es (q-his-cq-zone q))  (n->es (qrz "cqzone"))))
    (mvprintw 12 1 (format nil "ITU Zone:    ~10A ~10A ~10A~%"   (n->es (q-my-itu-zone q)) (n->es (q-his-itu-zone q)) (n->es (qrz "ituzone"))))
    (mvprintw 13 1 (format nil "DXCC:        ~10A ~10A ~10A~%"   (n->es (q-my-dxcc q))     (n->es (q-his-dxcc q))     (n->es (qrz "dxcc"))))
    (mvprintw 14 1 (format nil "IOTA:        ~10A ~10A ~10A~%"   (n->es (q-my-iota q))     (n->es (q-his-iota q))     (n->es (qrz "iota"))))
    (mvprintw 15 1 (format nil "Country:     ~10A ~10A ~10A~%"   (n->es (q-my-country q))  (n->es (q-his-country q))  (n->es (qrz "country"))))
    (mvprintw 16 1 (format nil "State:       ~10A ~10A ~10A~%"   (n->es (q-my-state q))    (n->es (q-his-state q))    (n->es (qrz "state"))))
    (mvprintw 17 1 (format nil "County:      ~10A ~10A ~10A~%"   (n->es (q-my-county q))   (n->es (q-his-county q))   (n->es (qrz "county"))))
    (mvprintw 18 1 (format nil "Grid:        ~10A ~10A ~10A~%"   (n->es (q-my-grid q))     (n->es (q-his-grid q))     (n->es (qrz "grid"))))
