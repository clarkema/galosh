...
(defun print-qso-details (q qrz-details)
  (labels ((qrz (field)
    ...
    (declare (inline qrz))
    (clear-partial-area)
    (mvprintw 1  1 (format nil "His call:    ~A~%"      (q-his-call q)))
    (mvprintw 2  1 (format nil "My call:     ~A~%"      (q-my-call q)))
    (mvprintw 4  1 (format nil "QSO date:    ~A~%"      (human-date (q-qso-date q))))
    (mvprintw 5  1 (format nil "QSO time:    ~A~%"      (q-time-on q)))
    (mvprintw 7  1 (format nil "Frequency:   ~A (~A)~%" (q-qrg q) (q-band q)))
    (mvprintw 8  1 (format nil "Mode:        ~A~%"      (q-mode q)))
    (mvprintw 9  1 (format nil "Tx / Rx RST: ~A ~A~%"   (q-tx-rst q) (q-rx-rst q)))
    
    (mvprintw 11 1 (format nil "             ~10A ~10A ~10A~%"   "Mine" "His"  "QRZ"))
    (mvprintw 12 1 (format nil "CQ Zone:     ~10A ~10A ~10A~%"   (n->es (q-my-cq-zone q))  (n->es (q-his-cq-zone q))  (n->es (qrz "cqzone"))))
    (mvprintw 13 1 (format nil "ITU Zone:    ~10A ~10A ~10A~%"   (n->es (q-my-itu-zone q)) (n->es (q-his-itu-zone q)) (n->es (qrz "ituzone"))))
    (mvprintw 14 1 (format nil "DXCC:        ~10A ~10A ~10A~%"   (n->es (q-my-dxcc q))     (n->es (q-his-dxcc q))     (n->es (qrz "dxcc"))))
    (mvprintw 15 1 (format nil "IOTA:        ~10A ~10A ~10A~%"   (n->es (q-my-iota q))     (n->es (q-his-iota q))     (n->es (qrz "iota"))))
    (mvprintw 16 1 (format nil "Country:     ~10A ~10A ~10A~%"   (n->es (q-my-country q))  (n->es (q-his-country q))  (n->es (qrz "country"))))
    (mvprintw 17 1 (format nil "State:       ~10A ~10A ~10A~%"   (n->es (q-my-state q))    (n->es (q-his-state q))    (n->es (qrz "state"))))
    (mvprintw 18 1 (format nil "County:      ~10A ~10A ~10A~%"   (n->es (q-my-county q))   (n->es (q-his-county q))   (n->es (qrz "county"))))
    (mvprintw 19 1 (format nil "Grid:        ~10A ~10A ~10A~%"   (n->es (q-my-grid q))     (n->es (q-his-grid q))     (n->es (qrz "grid"))))

    (mvprintw 21 1 (format nil "QSL Rx / Tx: ~A ~A~%"   (q-qsl-rcvd q) (q-qsl-sent q)))
    (refresh)))
...
(defun print-all-qsos (qsos)
  (if (> (length qsos) *LINES*)
      ...
      (let ((i 0))
	(dolist (qso qsos)
	  (mvprintw i 0 (format nil "~A ~A" (+ i 1) (qso-for-selection qso)))
	  (setf i (+ i 1))))))
	   ...
	   (let ((accessor-name (second form))
	     ...
	     (setf acc (append acc `(((char= ,c ,(first form))
				      (setf (,accessor-name ,val) (read-value :prompt ,(third form)
									      :buffer (mkstr (,accessor-name ,val))
									      ,@read-value-options))
  ...
  (let ((qrz-details (galosh-qrz:qrz-search (q-his-call qso))))
    (labels ((again (q)
	       ...
	       (case-with-char (code-char (getch)) qso
			       ...
			       (:func #\c #'(lambda () (merge-qso-qrz-details q qrz-details)))
